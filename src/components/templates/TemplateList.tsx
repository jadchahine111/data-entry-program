...
const fetchTemplates = async (): Promise<Template[]> => {
  const { data } = await axios.get<TemplateApiResponse[]>('http://localhost:8000/api/templates');
...
const createTemplate = async (template: Omit<Template, 'id' | 'recordCount' | 'createdAt'>): Promise<void> => {
  ...
  await axios.post('http://localhost:8000/api/templates', apiTemplate);
  // The API returns a status message, not the created template
  // We'll rely on refetching the templates after creation
};
...
const updateTemplate = async (template: Template): Promise<void> => {
  ...
  const apiTemplate = {
    ...
    fields: template.fields.map((field, index) => ({
      field_name: field.name,
      field_type: field.type,
      is_required: template.required ? 1 : 0, // Convert boolean to 0/1 for API
      display_order: index + 1,
      options: field.options // Include options if available
    }))
  };
  
  await axios.put(`http://localhost:8000/api/templates/${template.id}`, apiTemplate);
